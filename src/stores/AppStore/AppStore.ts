    ...
    *requestFitting(message: CARTA.IFittingRequest) {
    ...
    public *cloneWorkspace(name: string) {
        const workspace: Workspace = {
            workspaceVersion: 0,
            frontendVersion: CARTA_INFO.version,
            description: "Cloned workspace exported from CARTA",
            files: [],
            references: {},
            date: Date.now() / 1000
        };
	...
	const cloneWorkspace = yield this.apiService.cloneWorkspace(name, workspace);
        if (cloneWorkspace) {
...
/*
    async cloneWorkspace(name: string) {
        try {
            ...
            if (success) {
                AppToaster.show(SuccessToast("console", `Workspace ${name} cloned successfully.`, SnippetStore.ToasterTimeout));
                return;
            }
        } catch (err) {
            console.error(err);
